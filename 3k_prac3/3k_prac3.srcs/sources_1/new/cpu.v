...
localparam LITERAL = MEM_DATA_WIDTH;
    ...
    reg [MEM_DATA_WIDTH-1:0] mem_addr_write;
    ...
    localparam NOP = 0, INCR = 1, WRITE_REG = 2, MEM_TO_REG = 3, REG_TO_MEM = 4, JMP = 5, JMP_LESS = 6, REG_TO_REG = 7, SUB = 8, WRITE_MEM = 9;
    ...
    wire [POH_ADDR_WIDTH-1:0] cmd_addr2 = cmd_reg[PROG_DATA_WIDTH-COP_WIDTH-POH_ADDR_WIDTH -: POH_ADDR_WIDTH];
    wire [POH_ADDR_WIDTH-1:0] cmd_addr3 = cmd_reg[PROG_DATA_WIDTH-COP_WIDTH-2*POH_ADDR_WIDTH -: POH_ADDR_WIDTH];
    wire [LITERAL-1:0] literal = cmd_reg[LITERAL-1 -: LITERAL];
    wire [PROG_ADDR_WIDTH-1:0] addr_to_jmp = cmd_reg[PROG_DATA_WIDTH-COP_WIDTH-PROG_ADDR_WIDTH-1 -: PROG_ADDR_WIDTH];
    
    wire [POH_DATA_WIDTH-1:0] out_a, out_b;
    
    reg POH_wen;
    reg [POH_DATA_WIDTH-1:0] POH_AdrA, POH_AdrB, POH_w_addr;
    ...
    begin
        ...
        EXECUTE:
            case(cop)
                SUB: next_lz = POH_w_data[POH_DATA_WIDTH-1:0];
            default: next_lz <= lz + 1;
        endcase
    default:
        next_lz <= lz;
    endcase
